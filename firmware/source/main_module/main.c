...
GPIOInitConfig_t gpio_config[] = {
    ...
    GPIO_INIT_INPUT(NOT_PRECHARGE_COMPLETE_PORT, NOT_PRECHARGE_COMPLETE_PIN, GPIO_INPUT_PULL_DOWN),
...
// Thread Defines
DEFINE_CAN_TASKS();
FREERTOS_DEFINE_TASK(vehicle_fsm_periodic, VEHICLE_FSM_PERIOD_MS, TASK_PRIORITY_NORMAL, STACK_2048);
FREERTOS_DEFINE_TASK(fault_library_periodic, MAIN_MODULE_FAULT_SYNC_PERIOD_MS, TASK_PRIORITY_NORMAL, STACK_1024);
FREERTOS_DEFINE_TASK(SDC_task_periodic, SDC_TASK_PERIOD_MS, TASK_PRIORITY_NORMAL, STACK_512);
FREERTOS_DEFINE_TASK(report_telemetry_50hz, TELEMETRY_50HZ_PERIOD_MS, TASK_PRIORITY_LOW, STACK_512);
...
int main(void) {
    ...
    FREERTOS_START_TASK(vehicle_fsm_periodic);
    FREERTOS_START_TASK(fault_library_periodic);
    FREERTOS_START_TASK(SDC_task_periodic);
    FREERTOS_START_TASK(report_telemetry_50hz);
