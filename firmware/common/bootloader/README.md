...
| File | Responsibility |
| --- | --- |
| [`bootloader_common.h`](bootloader_common.h) | Protocol values, flash addresses, metadata, and reset API. |
| [`bootloader_common.c`](bootloader_common.c) | `.noinit` marker and generated CAN callbacks. |
...
Metadata is written only after a staged image is validated and installed.
`Bootloader_ResetForFirmwareDownload()` records a one-shot `.noinit` request
before resetting.
